...
const { sendMail, sendMailPassword, sendMailForgotPassword } = require('./../helpers/mail');
...
let userLoginWithGoogle = async (data) => {
    let verify = await client.verifyIdToken({ idToken: data.tokenId, audience: process.env.GOOGLE_CLIENT_ID });
    ...
    if (email_verified) {
        ...
        //Tao mat khau bat ki, sau do gui ve mail cho nguoi dung
        let password = randomString.generate(8);
        let salt = bcrypt.genSaltSync(saltRounds);
        let passwordHash = bcrypt.hashSync(password, salt);
        ...
        return { message: 'SUCCESS_LOGIN_WITH_GOOGLE', token: token, userId: newUser._id };
    }
}
