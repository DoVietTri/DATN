...
let addNewBanner = async (data) => {
    ...
    let responseUploadDetail = await cloudinary.uploader.upload(formatBufferToBase64(data.b_image).content, {
...
let deleteBannerById = async (id) => {
    ...
    let responseDestroyAvatar = await cloudinary.uploader.destroy(banner.b_image.public_id);
