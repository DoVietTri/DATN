...
const Dashboard = () => {
  ...
  useEffect(() => {
    dashboardAPI.countAllOrders().then(res => {
    ...
    dashboardAPI.countUserRegister().then(res => {
      setCountUserRegister(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.countAllProducts().then(res => {
      setCountAllProducts(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.salesDay().then(res => {
      setSalesDay(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.salesMonth().then(res => {
      setSalesMonth(res.data.data);
    }).catch(err => {
      console.log(err);
    });
    dashboardAPI.salesYear().then(res => {
      setSalesYear(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.statisticalOrderStatus().then(res => {
      setStatisticalOrderStatus(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.saleMonthOfYear().then(res => {
      setSaleMonthOfYear(res.data.data);
    }).catch(err => {
      console.log(err);
    });

    dashboardAPI.productsBestSeller().then(res => {
        ...
        <div className="container-fluid">
          ...
          <div className="row">
            ...
            <ItemBox
              ...
              detail={<small><Link to="/orders" className="font-weight-bold" > (Chi tiết)</Link></small>}
            />
            ...
            <ItemBox
              ...
              detail={<small><Link to="/customers" className="font-weight-bold" > (Chi tiết)</Link></small>}
            />
            ...
            <ItemBox
              ...
              detail={<small><Link to="/products" className="font-weight-bold" > (Chi tiết)</Link></small>}
            />
            ...
            <div className="col-12 col-sm-7 col-md-7">
              <div className="card card-info">
                <div className="card-header">
                  <h3 className="card-title">Biểu đồ doanh thu tháng trong năm</h3>
            ...
            <div className="col-12 col-sm-5 col-md-5">
              <div className="card card-blue">
                ...
                <div className="card-body">
                  <OrderStatus statisticalOrderStatus={statisticalOrderStatus} />
                </div>
              </div>
            </div>
                      ...
                      {productsBestSeller.map((v, i) => {
                        return (
                          <tr key={i}>
                            ...
                            <td>
                              <img src={ v.productDetail[0].p_image_detail.url } alt="product best seller" style={{ width: '50px' }} />
                            </td>
                            <td>{ v.quantity }</td>
                          </tr>
                        )
